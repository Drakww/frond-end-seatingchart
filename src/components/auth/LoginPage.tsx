...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => setShowText(true), 3000);        // mostrar texto
    setTimeout(() => setHideText(true), 4000);        // ocultar texto
    setTimeout(() => setShowForm(true), 5200);        // mostrar formulario
  }, []);
    ...
    <div className="bg-[#0F0F0F] w-screen h-screen flex items-center justify-center">
      <div
        ...
        style={{
          ...
          height: expandHeight ? "450px" : "10px",
        }}
      >
        {/* Texto inicial */}
        <div
          className={`text-[#E6E6E6] font-mono text-2xl transition-opacity duration-1000 absolute
    ${hideText ? "pointer-events-none" : ""}`}
          style={{
            opacity: hideText ? 0 : showText ? 1 : 0,
          }}
        >
          System Runwa7Fashion
        </div>
        ...
        >
          ...
          {
            SigninErrors.map((error: any, i: number) => (
